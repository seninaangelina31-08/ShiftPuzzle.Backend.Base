...
{ 
    ...
    const string ShowProductsMethod = "/store/show";
    ...
    static void DisplayProducts()
    {
        ...
        var response = client.GetAsync($"{url}:{port}{ShowProductsMethod}").Result;  
    ...
    {       
        ...
        var response = client.PostAsync($"{url}:{port}{AddProductMethod}", content).Result;
    ...
    {       
        ...
        var content = new StringContent(json, Encoding.UTF8, "application/json");
        var response = client.PostAsync($"{url}:{port}{AuthMethod}", content).Result;
        if (response.IsSuccessStatusCode)
        {
            var responseContent = response.Content.ReadAsStringAsync().Result;
            Console.WriteLine(responseContent);
