...
namespace PracticeA
{
    ...
    public class Bank
    {
        ...
        public bool RequestLoan(int accountNumber, double loanAmount)
        {
            ...
            if (account != null)
            {
                // Пример логики одобрения кредита (например, сумма кредита не превышает лимит)
        ...
        {
            var account = GetAccount(accountNumber);
            if (RequestLoan(accountNumber, amount))
            {
                account.Deposit(loanAmount);
                // Дополнительная логика обработки кредита, если необходимо
            }
