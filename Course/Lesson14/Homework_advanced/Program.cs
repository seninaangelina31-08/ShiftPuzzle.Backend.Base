    ...
    static async Task Main(string[] args)
    {
        ...
        using (StreamWriter outputFile = new StreamWriter("FREE_API.txt"))
        {
            foreach (var api in apiList.Entries)
            {
                ...
                {
                    await outputFile.WriteLineAsync($"ССЫЛКА: {api.Link}");
                    await outputFile.WriteLineAsync($"ОПИСАНИЕ: {api.Description}");
                    await outputFile.WriteLineAsync("АВТОРИЗАЦИЯ: не нужна (т.к. некоторые сервисы могут требовать авторизацию)");
